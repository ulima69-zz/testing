...
PROC SQL;
    ...
    CREATE TABLE EXCLUIR_REATIVADAS_1 AS  SELECT * FROM CONNECTION TO DB2(
        ...
        INNER JOIN DB2DEB.TDEB307 t2 ON (t1.CD_UOR_CC = t2.DEB307_AGENCIA AND t1.NR_CC = t2.DEB307_CONTA)
            ...
            AND (DT_ULT_RTVC_CT >= '01.07.2018' AND DT_ULT_RTVC_CT <= '31.12.2018')
...
((A.DT_FRMZ >= '01.07.2018' AND A.DT_FRMZ <= '31.12.2018') OR (A.DT_FRMZ < '01.07.2018' AND B.DT_FIM_PRTC >= '01.07.2018'))
     ...
     (MONTH(t1.DEB307_DT_ABERTURA) = (&MES_POSICAO.)) AND YEAR(t1.DEB307_DT_ABERTURA) = 2019
...
AND	((A.DT_FRMZ >= '01.07.2018' AND A.DT_FRMZ <= '31.12.2018') OR (A.DT_FRMZ < '01.07.2018' AND B.DT_FIM_PRTC >= '01.07.2018'))
            ...
            WHERE t1.CD_TIP_PSS = 2
            AND (MONTH(t1.DT_ULT_RTVC_CT) = (&MES_POSICAO.)) AND YEAR(t1.DT_ULT_RTVC_CT) = 2019
            AND t2.DEB307_COD_BDC <> 0
            AND t4.CD_CLI IS NULL
...
PROC SQL;
   ...
   CREATE TABLE BASE_TFA_2_ANT AS SELECT DISTINCT    
      ...
      AND ((MONTH(t1.DT_INI) = (&MES_POSICAO.)) AND YEAR(t1.DT_INI) = 2019) AND t1.DT_INI<= &D1.;
...
PROC STDIZE DATA=CLIENTES_PJ_BONUS_200 OUT=CLIENTES_PJ_BONUS_200 REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
...
PROC STDIZE DATA=CLIENTES_PJ_BONUS_1700 OUT=CLIENTES_PJ_BONUS_1700 REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
...
PROC SQL;
	CREATE TABLE PGT_FILTRADA AS
		...
		INNER JOIN (SELECT CD_CLI, CD_CTR_PGTO, MIN(DT_CTR_PGTO) AS K
					...
					WHERE YEAR(DT_CTR_PGTO) = &ANO_ATUAL. AND (MONTH(DT_CTR_PGTO) = (&MES_POSICAO.)) AND DT_CTR_PGTO <= &D1. 
		ORDER BY 1, 2;
QUIT;
...
PROC SQL;
      ...
      WHERE t2.COD_PROD = 539 AND (MONTH(t2.data) = (&MES_POSICAO.)) AND YEAR(t2.data) = &ANO_ATUAL. AND t2.data <= &D1.;
...
PROC STDIZE DATA=CONEX_JAN_ANT OUT=CONEX_JAN_ANT REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
...
PROC SQL;
   ...
   SELECT  
		  ...
		  t1.UOR,
		  t1.RM01 AS JAN_ATU,
          t1.CONTAGEM AS JAN_REPRO,
          (t1.CONTAGEM - t1.RM01) as DIFERENCA,
