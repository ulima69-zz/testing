...
PROC SQL;
    ...
    FROM (
        ...
        LEFT JOIN (
                SELECT DISTINCT 
			t1.CD_PSS_RLCD_IOR AS CD_CLI, 
			t2.CD_TIP_ANOT AS CD_TIP_ANOT_CADL,
			T2.QT_PESO_ANOT,
			T2.VL_ANOT_CADL
		FROM (SELECT NR_CTRE_SRF, CD_TIP_ANOT, VL_ANOT_CADL, CD_PESO_ANOT_EXNO AS QT_PESO_ANOT FROM DB2ACP.ANOT_CADL_EXNO_PSS WHERE CD_PESO_ANOT_EXNO IN (3, 4) 
		AND CD_TIP_ANOT IN (210, 195, 216, 217, 272, 82, 227, 126, 215, 277, 125, 153, 174, 270, 3, 271, 295, 165, 215, 277, 125, 153, 174, 270,
                        ...
                        303, 120, 66, 121, 83, 279, 234)
UNION SELECT NR_CTRE_SRF, CD_TIP_ANOT, VL_ANOT_CADL, CD_PESO_ANOT_ITNO AS QT_PESO_ANOT FROM DB2ACP.ANOT_CADL_ITNO_PSS  WHERE CD_PESO_ANOT_ITNO IN (3, 4)
AND CD_TIP_ANOT IN (210, 195, 216, 217, 272, 82, 227, 126, 215, 277, 125, 153, 174, 270, 3, 271, 295, 165, 215, 277, 125, 153, 174, 270,
                        3, 271, 295, 165, 246, 175, 128, 88, 65, 173, 85, 64, 68, 36, 304, 269, 275, 248, 39, 232, 233, 207, 224, 274, 230,    181, 10, 167, 211, 58,
                        303, 120, 66, 121, 83, 279, 234)) T2
			INNER JOIN DB2ACP.ANOT_PSS_CFMD_BCRO t1
				ON (t1.NR_CTRE_SRF = t2.NR_CTRE_SRF)
WHERE t1.CD_PSS_RLCD_IOR > 0
...
PROC SQL;
	...
	CREATE TABLE WORK.QUERY_FOR_ANOT_PSS_CFMD_BCRO AS SELECT * FROM CONNECTION TO DB2(
		SELECT DISTINCT 
			t1.CD_PSS_RLCD_IOR AS CD_CLI, 
			t2.CD_TIP_ANOT AS CD_TIP_ANOT_CADL,
			T2.QT_PESO_ANOT,
...
AND CD_TIP_ANOT IN (210, 195, 216, 217, 272, 82, 227, 126, 215, 277, 125, 153, 174, 270, 3, 271, 295, 165, 215, 277, 125, 153, 174, 270,
			...
			INNER JOIN DB2ACP.ANOT_PSS_CFMD_BCRO t1
				ON (t1.NR_CTRE_SRF = t2.NR_CTRE_SRF)
WHERE t1.CD_PSS_RLCD_IOR > 0) T2 ON (T1.MCI_FORNECEDOR = T2.CD_CLI)
...
PROC SQL;
   ...
   SELECT t2.PrefUEN, 
          ...
          t1.DV_CT_PGTO
      FROM AF.ORDENS_FINAIS_ENCARTEIRADOS t1
           LEFT JOIN TBLS.igrrede t2 ON (t1.PrefDep = t2.PrefDep);
QUIT;
...
PROC SQL;
   ...
   SELECT DISTINCT t1.PrefDep, 
          ...
          t1.MCI_FORNECEDOR, 
          t1.NOME_FONECEDOR,  
          t2.VL_LMCR_APVD AS LIM_APROV, 
          t2.VL_LMCR_UTZD AS LIM_UTIL, 
            (t2.VL_LMCR_APVD - t2.VL_LMCR_UTZD) AS Lim_disp
      FROM AF.ordens_finais_encarteirados t1
           LEFT JOIN BCS.LMCR_PJ_010716 t2 ON (t1.MCI_FORNECEDOR = t2.CD_CLI);
QUIT;
...
/*cer - endividamento*/
%ls(/dados/prep/bases/cer, out=work.cer_tbl);
data work.out_ls2;
	set work.cer_tbl;
...
proc sql;
	create table verificacao_diaria as 
		...
		from af.af_final t1, 
			tbls.dependencias t2, 
			tbls.dependencias t3
