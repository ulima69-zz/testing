...
/*Trocando a tabela de dependências*/
...
PROC SQL;
   CREATE TABLE WORK.CARTEIRAS AS SELECT
      ...
      FROM WORK.nova_dependencias t2
           INNER JOIN COMUM.PAI_REL_PJ_201810 t1 ON (t2.prefixo = t1.CD_PRF_DEPE)
...
PROC SQL;
   ...
   SELECT DISTINCT t2.CD_PRF_DEPE AS PREFIXO, 
      ...
      FROM DB2MIV.TX_MSG_EXPS_PJ t1
           INNER JOIN COMUM.PAI_REL_PJ_201810 t2 ON (t1.CD_CLI_MSG_EXPS = t2.CD_CLI)
      WHERE DATEPART(t1.TS_CRIC_MSG) >= MDY(MONTH('31OCT2018'd),1,YEAR('31OCT2018'd)) AND t1.NR_MTC_RPRT_CTRA = 0 
AND DATEPART(t1.TS_CRIC_MSG) <= '31OCT2018'd;
QUIT;
...
PROC SQL;
   ...
   SELECT DISTINCT t1.CD_CLI_MSG_EXPS AS mci, 
      ...
      FROM DB2MIV.TX_MSG_EXPS_PJ t1
	  INNER JOIN COMUM.PAI_REL_PJ_201810 t2 ON (t1.CD_CLI_MSG_EXPS = t2.CD_CLI)
      WHERE DATEPART(t1.TS_CRIC_MSG) >= MDY(MONTH('31OCT2018'd),1,YEAR('31OCT2018'd)) AND  DATEPART(t1.TS_CRIC_MSG) <= '31OCT2018'd
...
PROC SQL;
   ...
   SELECT t1.idGenesys, 
          ...
          t1.Transacoes, 
          t1.Vazio
      FROM UNC_ATA.relatorio_detalhado_102018 t1
      where t1.DataChamada<='31OCT2018'd; 
QUIT;
...
PROC SQL;
	...
	SELECT t1.CD_PRF_DEPE AS PREFIXO,
		   ...
		   t1.CD_CLI AS MCI
	FROM COMUM.PAI_REL_PJ_201810 t1
...
PROC SQL;
   ...
   SELECT t2.PREFIXO,
      ...
      FROM UNC_GAT.tel201810 t1
	  INNER JOIN WORK.MCI_CTRA t2 ON (t1.MCI = t2.MCI)
	  WHERE t1.statusAtend = 40	AND t1.horaSolicitacaoAtend BETWEEN "8:0:0"t AND "18:0:0"t AND t1.dataFimAtend<='31OCT2018'd;
QUIT;
...
and t1.Transferida = 't' AND t1.DataChamada<='31OCT2018'd;
QUIT;
...
PROC SQL;
   ...
   SELECT t1.idGenesys,
          ...
          t1.Vazio
      FROM UNC_ATA.RELATORIO_DETALHADO_102018 t1
      WHERE t1.DNIS = 2028 AND t1.DataChamada<='31OCT2018'd;
QUIT;
...
PROC SQL;
	...
	SELECT DISTINCT '31OCT2018'd FORMAT=DateMysql. AS POSICAO,
		   t1.PREFIXO,
		   t1.CARTEIRA,
		   (t3.QTD_CLIENTES) AS FC_CLI_OBJ,
		   (t3.QTD_CLI_USO_FC) AS FC_CLI_RLZ,
		   ((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) AS FC_UTILIZACAO,
		   ( IFN(((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) <= 5, 0, 0, 0) +
		   	 IFN(((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) > 5 AND ((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) <= 7.5, 5, 0, 0) +
			 IFN(((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) > 7.5 AND ((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) <= 10, 10, 0, 0) +
			 IFN(((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) > 10 AND ((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) <= 12.5, 15, 0, 0) +
			 IFN(((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) > 12.5 AND ((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) <= 15, 20, 0, 0) +
			 IFN(((t3.QTD_CLI_USO_FC / t3.QTD_CLIENTES)*100) > 15, 25, 0, 0)
		   ...
		   ( IFN((((t6.QTD_CHAMADAS - t6.QTD_CHAMADAS_N_ATENDIDAS) / t6.QTD_CHAMADAS)*100) <= 70, 0, 0, 0) +
		   	 IFN((((t6.QTD_CHAMADAS - t6.QTD_CHAMADAS_N_ATENDIDAS) / t6.QTD_CHAMADAS)*100) > 70 AND (((t6.QTD_CHAMADAS - t6.QTD_CHAMADAS_N_ATENDIDAS) / t6.QTD_CHAMADAS)*100) <= 75, 5, 0, 0) +
			 IFN((((t6.QTD_CHAMADAS - t6.QTD_CHAMADAS_N_ATENDIDAS) / t6.QTD_CHAMADAS)*100) > 75 AND (((t6.QTD_CHAMADAS - t6.QTD_CHAMADAS_N_ATENDIDAS) / t6.QTD_CHAMADAS)*100) <= 80, 10, 0, 0) +
...
%SumarizadorCNX( TblSASValores=CONSOLIDADO_CNX,  TblSASColunas=COLUNAS_SUMARIZAR,  NivelCTRA=1,  PAA_PARA_AGENCIA=0,  TblSaida=CONSOLIDADO_CNX, AAAAMM=201810); 
 

PROC SQL;
CREATE TABLE WORK.CONSOLIDADO_CNX AS 
SELECT POSICAO, PREFDEP, CTRA as Carteira, 201810 AS ANOMES, TP_CNX_AVL as TipDepCnx
FROM  WORK.CONSOLIDADO_CNX;
QUIT;
...
INNER JOIN IGR.IGRREDE_201810 t2
...
where aa_vl_aprd_in = 2018 and mm_vl_aprd_in = 10 and cd_In_mod_avlc = 11785;
QUIT;
...
where aa_vl_aprd_in = 2018 and mm_vl_aprd_in = 10 and  cd_in_mod_avlc = 11788;
QUIT;
...
PROC SQL;
CREATE TABLE auxiliar.NOVO_CONSOLIDADO_CNX_1_10 AS 
SELECT *
...
PROC SQL;
    CREATE TABLE CONEXAO AS
        SELECT
            ...
            ||'F7176219'
            ||COMPRESS(PUT('31OCT2018'd, ddmmyy10.))
			||'N' AS L
        FROM WORK.NOVO_CONSOLIDADO_CNX_1 t1       
;QUIT;


%GerarBBM(TabelaSAS=CONEXAO, Caminho=/dados/infor/transfer/enviar/, ExtencaoBBM=M1062);
...
PROC SQL;
CREATE TABLE auxiliar.CONSOLIDADO_REL_10 AS 
SELECT *
FROM  WORK.CONSOLIDADO_REL;
QUIT;
