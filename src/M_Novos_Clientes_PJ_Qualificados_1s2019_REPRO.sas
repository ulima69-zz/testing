...
PROC SQL;
    CONNECT TO DB2 (AUTHDOMAIN=DB2SVARC DATABASE=DB23P41);
    CREATE TABLE EXCLUIR_REATIVADAS_1 AS  SELECT * FROM CONNECTION TO DB2(
        ...
        INNER JOIN DB2DEB.TDEB307 t2 ON (t1.CD_UOR_CC = t2.DEB307_AGENCIA AND t1.NR_CC = t2.DEB307_CONTA)
            ...
            AND DT_ULT_RTVC_CT >= '01.07.2018' AND DT_ULT_RTVC_CT < '02.01.2019'
...
PROC SQL;CONNECT TO DB2 (AUTHDOMAIN=DB2SVARC DATABASE=DB23P41);
   ...
   SELECT t1.DEB307_AGENCIA as CD_UOR_CC,
      ...
      AND ( (A.DT_FRMZ >= '01.07.2018' and A.DT_FRMZ < '02.01.2019') OR (A.DT_FRMZ < '01.07.2018' AND B.DT_FIM_PRTC >= '01.07.2018'))

	  
)  T4 ON (T1.DEB307_COD_BDC = T4.CD_CLI)
      WHERE MONTH(t1.DEB307_DT_ABERTURA) = 1 AND YEAR(t1.DEB307_DT_ABERTURA) = 2019 AND t1.DEB307_COD_BDC <> 0
        ...
        INNER JOIN DB2DEB.TDEB307 t2 ON (t1.CD_UOR_CC = t2.DEB307_AGENCIA AND t1.NR_CC = t2.DEB307_CONTA)
         ...
         LEFT JOIN (SELECT B.CD_PSS_CTR_OPR AS CD_CLI
          ...
          AND ( (A.DT_FRMZ >= '01.07.2018' and A.DT_FRMZ < '02.01.2019') OR (A.DT_FRMZ < '01.07.2018' AND B.DT_FIM_PRTC >= '01.07.2018'))
            ...
            WHERE t1.CD_TIP_PSS = 2
            AND MONTH(DT_ULT_RTVC_CT) = 1 AND YEAR(DT_ULT_RTVC_CT) = 2019
            AND t2.DEB307_COD_BDC <> 0
            AND t4.CD_CLI IS NULL
...
PROC SQL;
   ...
   CREATE TABLE BASE_TFA_2_ANT AS SELECT DISTINCT    
      ...
      FROM BASE_TFA t1
      WHERE t1.COD_PCTE IN (1001 1030 1002 1031 1027 1032 1028 1033) AND t1.DT_FIM > '31JAN2019'd 
      AND MONTH(t1.DT_INI) = 1 AND YEAR(t1.DT_INI) = 2019;
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_PJ_AGRUPANDO_1 AS SELECT DISTINCT  
	...
	MAX(t1.NR_SEQL_FATM) AS NR_SEQL_FATM
	  ...
	  WHERE MONTH(t1.DT_REF_FATM) = 01 AND YEAR(t1.DT_REF_FATM) = 2019
	  GROUP BY 1
	  ORDER BY 1
      ;
...
/*            1=MOBILE, 2=GEFIN*/
        ...
        FROM CONNECTION TO DB2(
            ...
            WHERE
                ...
                AND DATE(t1.TS_INRO_CLI) >= '02.01.2019' 
                AND DATE(t1.TS_INRO_CLI) <= '31.01.2019' 
                AND t2.COD_TIPO = 2
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_PJ_4 AS 
      ...
      LEFT JOIN COMUM.PAI_REL_201901 t2 ON (t1.MCI = t2.CD_CLI)
      GROUP BY 1, 2, 3
      ORDER BY 1, 2, 3;
...
PROC SQL;
      ...
      SELECT DISTINCT 
	     ...
	     t1.MCI, 
         SUM(t1.M_201901) AS M_201901

      FROM BON11.resumo_faturamento_2019 t1
	  WHERE t1.M_201901 IS NOT MISSING AND M_201901 > 0
...
PROC SQL;
      ...
      SELECT DISTINCT 
          ...
          t1.PREFDEP, 
          t1.CARTEIRA,
		  IFC(t1.M_201901 > 0, "Sim", "Não") AS CIELO
...
PROC SQL;
      ...
      SELECT DISTINCT 
	     ...
	     t1.CD_CLI AS MCI, 
         SUM(t2.F_201901) AS F_201901

      FROM BON12.BASE_CADASTRO t1 
      LEFT JOIN BON12.base_faturamento_2019 t2 ON t1.ID_STELO = t2.ID_STELO
	  WHERE t2.F_201901 IS NOT MISSING AND t1.CD_CLI IS NOT MISSING AND t2.F_201901 > 0
...
PROC SQL;
      ...
      SELECT DISTINCT 
         ...
         t1.CARTEIRA,
		 t1.CIELO,
		 IFC(t1.F_201901 > 0, "Sim", "Não") AS STELO
...
PROC SQL;
	CREATE TABLE PGT_FILTRADA AS
		...
		INNER JOIN (SELECT CD_CLI, CD_CTR_PGTO, MIN(DT_CTR_PGTO) AS K
					...
					WHERE YEAR(DT_CTR_PGTO) = 2019 AND MONTH(DT_CTR_PGTO) = 1  
		ORDER BY 1, 2;
QUIT;
...
PROC SQL;
      ...
      WHERE t2.COD_PROD = 539 AND MONTH(t2.data) = 1 AND YEAR(t2.data) = 2019;
...
%BuscarPrefixosIndicador(IND=180, MMAAAA=012019, NIVEL_CTRA=1, SO_AG_PAA=1);
...
LEFT JOIN IGR.IGRREDE_201901 t2 ON t1.PREFDEP = input(t2.PREFDEP, 4.)
GROUP BY 1,2,3;
QUIT;
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_CARTEIRAS AS 
	...
	WHERE t1.CARTEIRA IS NOT MISSING
	ORDER BY 1, 2	  
    ;
...
PROC SQL;
   ...
   SELECT distinct INPUT(t2.PREFAGENC, 4.) AS PREFDEP, 0 AS CTRA, MCI, INPUT(t2.UOR, 9.) AS UOR, CONTAGEM
          ...
          FROM CLIENTES_PREFIXO t1
		  INNER JOIN IGR.IGRREDE_201901 t2 ON t1.PREFDEP = input(t2.PREFDEP, 4.)
...
PROC SQL;
   ...
   SELECT  
       ...
       FROM COX.indicador_000000180 T1
 	   WHERE t1.COMP = 0 AND RM01 IS NOT MISSING
	   ORDER BY 1,2;
QUIT;


PROC STDIZE DATA=CONEX_JAN OUT=CONEX_JAN REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;


PROC SQL;
   CREATE TABLE CONEX_FEV AS 
   SELECT  
...
PROC SQL;
   ...
   SELECT  
          ...
          t1.CTRA,
		  t1.UOR,
		  IFN(t1.RM01 > t1.CONTAGEM, t1.RM01, t1.CONTAGEM) AS CONTAGEM		 
...
PROC SQL;
    CREATE TABLE WORK.PARA_BASE_CONEXAO_COMP AS
        SELECT
			...
			'31JAN2019'd  FORMAT YYMMDD10. AS POSICAO /*DATA DO VALOR LEVANTADO*/
...
%BuscarPrefixosIndicador(IND=180, MMAAAA=012019, NIVEL_CTRA=0, SO_AG_PAA=1);
...
PROC SQL;
   ...
   CREATE TABLE REL_PREFIXO_PAA AS 
      ...
      FROM REL_PREFIXO t1
	      INNER JOIN IGR.IGRREDE_201901 t2 ON t1.AGENCIA = input(t2.PREFDEP, 4.)
		  INNER JOIN PREFIXOS_IND_000000180 t3 ON input(t2.PREFAGENC, 4.) = t3.PREFDEP
		  WHERE input(t2.PREFAGENC, 4.) <> 0
      ORDER BY 1;
...
FROM REL_PREFIXO t1
LEFT JOIN IGR.IGRREDE_201901 t2 ON t1.AGENCIA = input(t2.PREFDEP, 4.)
GROUP BY 1,2,3;
QUIT;
...
WHERE t1.MMAAAA = 012019 AND t1.CTRA = 0;
