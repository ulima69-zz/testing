...
PROC SQL;
   CREATE TABLE CONEX_PROV_FEV AS 
   SELECT DISTINCT 

          t1.cli as MCI		  

       FROM COX.cli_ind_000000179_dt022019 T1
 	   WHERE t1.COMP = 1
	   ORDER BY 1;
QUIT;
...
PROC SQL;
   ...
   SELECT DISTINCT 
          ...
          IFC(t2.MCI IS NOT MISSING, "Janeiro", IFC(t3.MCI IS NOT MISSING, "Fevereiro",

          IFC(t1.QUALIFICADO = "Sim" AND MONTH(t1.DT_ABERTURA) = 3, "Março", "Não Qualificado" ))) AS MES_QUAL

      FROM TABELA_INTERMEDIARIA_2 t1

	  LEFT JOIN CONEX_PROV_JAN t2 ON t1.MCI = t2.MCI
	  LEFT JOIN CONEX_PROV_FEV t3 ON t1.MCI = t3.MCI
...
PROC SQL;
   ...
   SELECT DISTINCT 
	  ...
	  WHERE t1.QUALIFICADO = "Sim" and t1.MES_QUAL = "Março"
...
%ExportarCNX_CLI(IND=179, MMAAAA=&MESANO);
%ExportarCNX_IND(IND=179, MMAAAA=&MESANO, ORC=0, RLZ=1); 
%ExportarCNX_COMP(IND=179, MMAAAA=&MESANO, ORC=0, RLZ=1);
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_REL_1 AS 
   SELECT DISTINCT 
          ...
          t1.DT_ABERTURA FORMAT ddmmyy10.,		 
          t1.PERFIL,
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_REL_2 AS 
   SELECT DISTINCT 
      ...
      FROM CLIENTES_REL_1 t1
	  WHERE t1.MES_QUAL <> "Janeiro"
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_REL_PAA AS 
   SELECT DISTINCT           
...
/*TRAZENDO OS CLIENTES QUALIFICADOS EM JANEIRO*/
/*TRAZENDO OS CLIENTES QUALIFICADOS EM JANEIRO*/
/*TRAZENDO OS CLIENTES QUALIFICADOS EM JANEIRO*/
/*TRAZENDO OS CLIENTES QUALIFICADOS EM JANEIRO*/
...
PROC SQL;
   ...
   CREATE TABLE MCI_QUAL_JAN_2 AS 
   SELECT DISTINCT 
      ...
      FROM CONEXAO_CLI t1
	  left JOIN MCI_QUAL_JAN t2 ON t1.MCI = t2.MCI
...
DATA CLIENTES_REL_FINAL;
	MERGE CLIENTES_REL_2 CLIENTES_REL_PAA MCI_QUAL_JAN_2;
	BY PREFIXO MCI ;
RUN;
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_REL_FINAL AS 
   SELECT DISTINCT           
...
PROC SQL;
   CREATE TABLE CLIENTES_BASE_1 AS 
   SELECT DISTINCT 

          t1.PREFIXO, 
          t1.MCI,

		  IFN(t1.SEGMENTO_ABC = "Sim", 1, 0) AS TOTAL,
		  		  
		  IFN(t1.APPHAB = "Sim", 1, 0) AS APPHAB_SIM,
		  IFN(t1.APPHAB = "Não", 1, 0) AS APPHAB_NAO,

		  IFN(t1.RESTRICAO = "Sim", 1, 0) AS RESTRICAO_SIM,
		  IFN(t1.RESTRICAO = "Não", 1, 0) AS RESTRICAO_NAO,

          IFN(t1.LIMICRED = "Sim", 1, 0) AS LIMICRED_SIM,
		  IFN(t1.LIMICRED = "Não", 1, 0) AS LIMICRED_NAO,

          IFN(t1.ENCARTEIRADO =  "Sim",  1, 0) AS ENCARTEIRADO_SIM,
		  IFN(t1.ENCARTEIRADO =  "Não",  1, 0) AS ENCARTEIRADO_NAO,

		  IFN(t1.QUALIFICADO = "Sim", 1, 0) AS QUALIFICADO_SIM,
		  IFN(t1.QUALIFICADO = "Sim" AND t1.MES_QUAL = "Janeiro" , 1, 0) AS QUALIFICADO_JAN_SIM,

		  IFN(t1.QUALIFICADO = "Não", 1, 0) AS QUALIFICADO_NAO,

		  IFN(t1.CLIENTE_NOVO = "Sim", 1, 0) AS CLIENTE_NOVO_SIM,
		  IFN(t1.CLIENTE_NOVO = "Não", 1, 0) AS CLIENTE_NOVO_NAO,

		  IFN(t1.PREF_IND <> 0, 1, 0) AS PREF_IND_SIM,
		  IFN(t1.PREF_IND = 0, 1, 0) AS PREF_IND_NAO
...
PROC SQL;
   ...
   SELECT DISTINCT 
          ...
          t1.PREFIXO, 
          SUM(TOTAL) AS TOTAL,
		  SUM(t1.APPHAB_SIM) AS APPHAB_SIM,
		  SUM(t1.APPHAB_NAO) AS APPHAB_NAO,
		  SUM(t1.RESTRICAO_SIM) AS RESTRICAO_SIM,
		  SUM(t1.RESTRICAO_NAO) AS RESTRICAO_NAO,
          SUM(t1.LIMICRED_SIM) AS LIMICRED_SIM,
		  SUM(t1.LIMICRED_NAO) AS LIMICRED_NAO,
          SUM(t1.ENCARTEIRADO_SIM) AS ENCARTEIRADO_SIM,
		  SUM(t1.ENCARTEIRADO_NAO) AS ENCARTEIRADO_NAO,
		  SUM(t1.QUALIFICADO_SIM) AS QUALIFICADO_SIM,
		  SUM(t1.QUALIFICADO_JAN_SIM) AS QUALIFICADO_JAN_SIM,
		  SUM(t1.QUALIFICADO_NAO) AS QUALIFICADO_NAO,
		  SUM(t1.CLIENTE_NOVO_SIM) AS CLIENTE_NOVO_SIM,
		  SUM(t1.CLIENTE_NOVO_NAO) AS CLIENTE_NOVO_NAO,
		  SUM(t1.PREF_IND_SIM) AS PREF_IND_SIM,
		  SUM(t1.PREF_IND_NAO) AS PREF_IND_NAO

      FROM CLIENTES_BASE_1 t1
	  GROUP BY 1
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_PREFIXO_PAA_ANT AS 
   SELECT DISTINCT           
          
          INPUT(t2.PREFAGENC, 4.) AS PREFIXO, 
		  t1.TOTAL AS TOTAL_1,
		  t1.APPHAB_SIM AS APPHAB_SIM_1,
		  t1.APPHAB_NAO AS APPHAB_NAO_1,
		  t1.RESTRICAO_SIM AS RESTRICAO_SIM_1,
		  t1.RESTRICAO_NAO AS RESTRICAO_NAO_1,
          t1.LIMICRED_SIM AS LIMICRED_SIM_1,
		  t1.LIMICRED_NAO AS LIMICRED_NAO_1,
          t1.ENCARTEIRADO_SIM AS ENCARTEIRADO_SIM_1,
		  t1.ENCARTEIRADO_NAO AS ENCARTEIRADO_NAO_1,
		  t1.QUALIFICADO_SIM AS QUALIFICADO_SIM_1,
		  t1.QUALIFICADO_JAN_SIM AS QUALIFICADO_JAN_SIM_1,		  
		  t1.QUALIFICADO_NAO AS QUALIFICADO_NAO_1,
		  t1.CLIENTE_NOVO_SIM AS CLIENTE_NOVO_SIM_1,
		  t1.CLIENTE_NOVO_NAO AS CLIENTE_NOVO_NAO_1,
		  t1.PREF_IND_SIM AS PREF_IND_SIM_1,
		  t1.PREF_IND_NAO AS PREF_IND_NAO_1
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_PREFIXO_PAA AS 
   SELECT DISTINCT           
          
          t1.PREFIXO, 
		  SUM(t1.TOTAL_1) AS TOTAL_1,
		  SUM(t1.APPHAB_SIM_1) AS APPHAB_SIM_1,
		  SUM(t1.APPHAB_NAO_1) AS APPHAB_NAO_1,
		  SUM(t1.RESTRICAO_SIM_1) AS RESTRICAO_SIM_1,
		  SUM(t1.RESTRICAO_NAO_1) AS RESTRICAO_NAO_1,
          SUM(t1.LIMICRED_SIM_1) AS LIMICRED_SIM_1,
		  SUM(t1.LIMICRED_NAO_1) AS LIMICRED_NAO_1,
          SUM(t1.ENCARTEIRADO_SIM_1) AS ENCARTEIRADO_SIM_1,
		  SUM(t1.ENCARTEIRADO_NAO_1) AS ENCARTEIRADO_NAO_1,
		  SUM(t1.QUALIFICADO_SIM_1) AS QUALIFICADO_SIM_1,
          SUM(t1.QUALIFICADO_JAN_SIM_1) AS QUALIFICADO_JAN_SIM_1,		  
		  SUM(t1.QUALIFICADO_NAO_1) AS QUALIFICADO_NAO_1,
		  SUM(t1.CLIENTE_NOVO_SIM_1) AS CLIENTE_NOVO_SIM_1,
		  SUM(t1.CLIENTE_NOVO_NAO_1) AS CLIENTE_NOVO_NAO_1,
		  SUM(t1.PREF_IND_SIM_1) AS PREF_IND_SIM_1,
		  SUM(t1.PREF_IND_NAO_1) AS PREF_IND_NAO_1

      FROM CLIENTES_PREFIXO_PAA_ANT t1
...
DATA PREFIXOS;
	MERGE CLIENTES_PREFIXO CLIENTES_PREFIXO_PAA;
	BY PREFIXO;
RUN;


PROC STDIZE DATA=PREFIXOS OUT=PREFIXOS REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
...
PROC SQL;
   ...
   CREATE TABLE PREFIXOS_1 AS 
   SELECT DISTINCT t1.PREFIXO,

          (TOTAL + TOTAL_1) AS TOTAL,
		  (APPHAB_SIM + APPHAB_SIM_1) AS APPHAB_SIM,
		  (APPHAB_NAO + APPHAB_NAO_1) AS APPHAB_NAO,
		  (RESTRICAO_SIM + RESTRICAO_SIM_1) AS RESTRICAO_SIM,
		  (RESTRICAO_NAO + RESTRICAO_NAO_1) AS RESTRICAO_NAO,
          (LIMICRED_SIM + LIMICRED_SIM_1) AS LIMICRED_SIM,
		  (LIMICRED_NAO + LIMICRED_NAO_1) AS LIMICRED_NAO,
          (ENCARTEIRADO_SIM + ENCARTEIRADO_SIM_1) AS ENCARTEIRADO_SIM,
		  (ENCARTEIRADO_NAO + ENCARTEIRADO_NAO_1) AS ENCARTEIRADO_NAO,
		  (QUALIFICADO_SIM + QUALIFICADO_SIM_1) AS QUALIFICADO_SIM,
		  (QUALIFICADO_JAN_SIM + QUALIFICADO_JAN_SIM_1) AS QUALIFICADO_JAN_SIM,		  
		  (QUALIFICADO_NAO + QUALIFICADO_NAO_1) AS QUALIFICADO_NAO,
		  (CLIENTE_NOVO_SIM + CLIENTE_NOVO_SIM_1) AS CLIENTE_NOVO_SIM,
		  (CLIENTE_NOVO_NAO + CLIENTE_NOVO_NAO_1) AS CLIENTE_NOVO_NAO,
		  (PREF_IND_SIM + PREF_IND_SIM_1) AS PREF_IND_SIM,
		  (PREF_IND_NAO + PREF_IND_NAO_1) AS PREF_IND_NAO
...
PROC SQL;
CREATE TABLE TABELA_AUX AS SELECT DISTINCT t1.*, t2.PREFSUPREG, t2.PREFSUPEST, t2.PREFUEN
FROM CLIENTES_PREFIXO t1
LEFT JOIN IGR.IGRREDE_&ANOMES. t2 ON t1.PREFIXO = input(t2.PREFDEP, 4.)
GROUP BY 1,2,3;
QUIT;
...
PROC SQL;
CREATE TABLE TABELA_FINAL_GEREV AS SELECT 

          INPUT(t1.PREFSUPREG, 4.) AS PREFIXO, 
          SUM(TOTAL) AS TOTAL,
		  SUM(t1.APPHAB_SIM) AS APPHAB_SIM,
		  SUM(t1.APPHAB_NAO) AS APPHAB_NAO,
		  SUM(t1.RESTRICAO_SIM) AS RESTRICAO_SIM,
...
PROC SQL;
CREATE TABLE TABELA_FINAL_SUPER AS SELECT 

          INPUT(t1.PREFSUPEST, 4.) AS PREFIXO, 
          SUM(TOTAL) AS TOTAL,
		  SUM(t1.APPHAB_SIM) AS APPHAB_SIM,
		  SUM(t1.APPHAB_NAO) AS APPHAB_NAO,
		  SUM(t1.RESTRICAO_SIM) AS RESTRICAO_SIM,
...
PROC SQL;
CREATE TABLE TABELA_FINAL_SUPPJPF AS SELECT 

          INPUT(t1.PREFUEN, 4.) AS PREFIXO, 
          SUM(TOTAL) AS TOTAL,
		  SUM(t1.APPHAB_SIM) AS APPHAB_SIM,
		  SUM(t1.APPHAB_NAO) AS APPHAB_NAO,
		  SUM(t1.RESTRICAO_SIM) AS RESTRICAO_SIM,
...
PROC SQL;
CREATE TABLE TABELA_FINAL_DIR AS SELECT 
...
PROC SQL;
CREATE TABLE TABELA_FINAL_VICE AS SELECT 
...
PROC SQL;
   CREATE TABLE CLIENTES_RELATORIO_FINAL_1 AS 
   SELECT DISTINCT 
...
/*********************************************/
/*********************************************/
/*******TRAZENDO JANEIRO DO CONEXAO***********/
/*********************************************/
/*********************************************/


PROC SQL;
   CREATE TABLE CONEX_JANEIRO AS 
   SELECT DISTINCT 

          t1.PREFDEP,
		  MAX(t1.RM01) AS QUALIFICADO_SIM_JAN_CONEX

       FROM COX.INDICADOR_000000179 t1	
       INNER JOIN IGR.IGRREDE_&ANOMES. t2 ON t1.PREFDEP = input(t2.PREFDEP, 4.) AND T1.UOR = INPUT(t2.UOR, 9.)
	   WHERE t1.COMP = 0 AND t1.CTRA = 0 
	   GROUP BY 1
	   ORDER BY 1;
QUIT;


PROC STDIZE DATA=CONEX_JANEIRO OUT=CONEX_JANEIRO REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;


PROC SQL;
   CREATE TABLE CLIENTES_RELATORIO_FINAL_2 AS 
   SELECT DISTINCT 

          t1.PREFIXO AS PREFDEP, 
		  (t1.TOTAL - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS TOTAL,     
          (t1.CLIENTE_NOVO_SIM - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS CLIENTE_NOVO_SIM,
		  t1.CLIENTE_NOVO_NAO, 
		  (t1.RESTRICAO_SIM - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS RESTRICAO_SIM,
		  t1.RESTRICAO_NAO,
		  (t1.LIMICRED_SIM - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS LIMICRED_SIM,
		  t1.LIMICRED_NAO,
		  (t1.PREF_IND_SIM - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS PREF_IND_SIM,
		  t1.PREF_IND_NAO,           
          (t1.ENCARTEIRADO_SIM - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS ENCARTEIRADO_SIM,
		  t1.ENCARTEIRADO_NAO,
		  (t1.APPHAB_SIM - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS APPHAB_SIM,
		  t1.APPHAB_NAO,
		  (t1.QUALIFICADO_SIM - t1.QUALIFICADO_JAN_SIM + t2.QUALIFICADO_SIM_JAN_CONEX) AS QUALIFICADO_SIM,	  
		  t1.QUALIFICADO_NAO		  

      FROM TABELA_FINAL t1
	  LEFT JOIN CONEX_JANEIRO T2 ON t1.PREFIXO = t2.PREFDEP
	  GROUP BY 1
...
PROC SQL;
   CREATE TABLE CLIENTES_RELATORIO_FINAL_3 AS 
   SELECT DISTINCT 

          t1.PREFDEP, 
		  t1.TOTAL,     
          IFN(t1.CLIENTE_NOVO_SIM >= 0, t1.CLIENTE_NOVO_SIM, 0) AS CLIENTE_NOVO_SIM,
		  t1.CLIENTE_NOVO_NAO, 
		  IFN(t1.RESTRICAO_SIM >= 0, t1.RESTRICAO_SIM, 0) AS RESTRICAO_SIM,
		  t1.RESTRICAO_NAO,
		  IFN(t1.LIMICRED_SIM  >= 0, t1.LIMICRED_SIM, 0) AS LIMICRED_SIM,
		  t1.LIMICRED_NAO,
		  IFN(t1.PREF_IND_SIM >= 0, t1.PREF_IND_SIM, 0) AS PREF_IND_SIM,
		  t1.PREF_IND_NAO,           
          IFN(t1.ENCARTEIRADO_SIM >= 0, t1.ENCARTEIRADO_SIM, 0) AS ENCARTEIRADO_SIM,
		  t1.ENCARTEIRADO_NAO,
		  IFN(t1.APPHAB_SIM >= 0, t1.APPHAB_SIM, 0) AS APPHAB_SIM,
		  t1.APPHAB_NAO,
		  IFN(t1.QUALIFICADO_SIM >= 0, t1.QUALIFICADO_SIM, 0) AS QUALIFICADO_SIM,	  
		  t1.QUALIFICADO_NAO		  

      FROM CLIENTES_RELATORIO_FINAL_2 t1
	  GROUP BY 1
...
data CLIENTES_RELATORIO_FINAL;
format POSICAO yymmdd10.;
set CLIENTES_RELATORIO_FINAL_3;
posicao = &D1;
run;


PROC STDIZE DATA=CLIENTES_RELATORIO_FINAL OUT=CLIENTES_RELATORIO_FINAL REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
...
%LET Keypass=prospeccao-clientes-iiAi92FDmsXsaDUHX8nC2JLFnqb9NJROOtPhywlqOuVXwjesAV;
%LET Rotina=prospeccao-clientes;
%ProcessoIniciar();
...
PROC SQL;
	...
	INSERT INTO TABELAS_EXPORTAR_REL VALUES('CLIENTES_RELATORIO_FINAL', 'prospeccao-clientes');
	INSERT INTO TABELAS_EXPORTAR_REL VALUES('CLIENTES_REL_FINAL', 'detalhe');
   ;
QUIT;
...
x cd /dados/infor/producao/prospeccao_pf_19 ;
x cd /dados/publica/b_dados ;
x cd /dados/gecen/interno/bases/rel ;
x cd /dados/infor/conexao/2019/000000179 ;
