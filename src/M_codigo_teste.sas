...
CALL SYMPUT('DIA',"'"||Put(D1, yymmdd10.)||"'");
...
%PUT &d1. &DIA. &ANOMES. &MMAAAA. &INI_MES. &ANOMES_ANT.;
...
PROC SQL;
   ...
   SELECT DISTINCT PREFIXO
      FROM TRPJ.AGENCIAS_FALECOM_&ANOMES_ANT.
WHERE FALECOM=1;
QUIT;
...
WHERE (datepart(t1.TS_CRIC_MSG)) ne &Excluir_data. /*and datepart(t1.TS_CRIC_MSG) = &D1.*/;
;QUIT;
...
PROC SQL;
	CREATE TABLE INTERACAO_CLIENTE AS 
		SELECT t1.PREFDEP, 
			t1.CARTEIRA, 
			t1.CD_CLI, 
			ifn((hora_cliente>'17:0:0't),diaUtilPosterior(data),data) format ddmmyy10. as  data, 
		(case 
...
PROC SQL;
	CREATE TABLE WORK.INTERACAO_BB AS 
		SELECT DISTINCT
			...
			t1.CARTEIRA, 
			t1.CD_CLI, 
			ifn((hora>'17:0:0't),diaUtilPosterior(data),data) format ddmmyy10. as  data, 
		(case 
...
PROC SQL;
   ...
   SELECT t2.CD_PRF_DEPE,
	  ...
	  and dataSolicitacaoAtend ne &Excluir_data. /* and dataSolicitacaoAtend = &D1.*/
        ;
QUIT;
...
PROC SQL;
   ...
   t1.CTRA
      ...
      FROM CONEXAO_&ANOMES  t1	  
	  LEFT JOIN AUX_TP.MOV_14 T2 ON t1.PREFDEP = t2.PREFDEP and t1.CTRA = t2.CTRA
	  WHERE t2.PREFDEP IS MISSING AND t2.CTRA IS MISSING AND t1.CTRA <> 0
...
PROC SQL;
   CREATE TABLE MEDICAO_1 AS 
   SELECT 

   t1.PREFDEP, 
   t1.CTRA,
   t2.REALIZADO - t1.REALIZADO as QUEDA_13_14
          
      FROM AUX_TP.CONEXAO_TRPJ_DIA13 t1	  
	  INNER JOIN AUX_TP.CONEXAO_TRPJ_DIA14 t2 ON t1.PREFDEP = t2.PREFDEP and t1.CTRA = t2.CTRA
	  WHERE (t2.REALIZADO - t1.REALIZADO) < 0 AND t1.CTRA <> 0
	  ORDER BY 1,2
;
QUIT;
...
PROC SQL;
   CREATE TABLE MEDICAO_2 AS 
   SELECT 

   t1.PREFDEP, 
   t1.CTRA,
   t2.REALIZADO - t1.REALIZADO as AUMENTO_FORA_14
          
      FROM AUX_TP.CONEXAO_TRPJ_COM14 t1	  
	  INNER JOIN AUX_TP.CONEXAO_TRPJ_SEM14 t2 ON t1.PREFDEP = t2.PREFDEP and t1.CTRA = t2.CTRA
	  WHERE (t2.REALIZADO - t1.REALIZADO) > 0 AND t1.CTRA <> 0
	  ORDER BY 1,2
      ;
QUIT;
...
PROC SQL;
   CREATE TABLE MEDICAO_FINAL AS 
   SELECT 

   IFN(t1.PREFDEP IS NOT MISSING, t1.PREFDEP, IFN(t2.PREFDEP IS NOT MISSING, t2.PREFDEP, t3.PREFDEP)) AS PREFDEP,
   IFN(t1.CTRA IS NOT MISSING, t1.CTRA, IFN(t2.CTRA IS NOT MISSING, t2.CTRA, t3.CTRA)) AS CTRA,
   IFC(t3.PREFDEP IS NOT NULL AND t3.CTRA IS NOT NULL, "NÃO", "SIM") AS SEM_MOV_14,
   t1.QUEDA_13_14,
   t2.AUMENTO_FORA_14 
             
      FROM MEDICAO_1 t1	  
	  FULL JOIN  MEDICAO_2 t2 ON t1.PREFDEP = t2.PREFDEP and t1.CTRA = t2.CTRA
	  FULL JOIN  SEM_MOVIMENTO t3 ON t1.PREFDEP = t3.PREFDEP and t1.CTRA = t3.CTRA
	  ORDER BY 1,2
      ;
QUIT;


PROC STDIZE DATA=MEDICAO_FINAL OUT=MEDICAO_FINAL REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
