...
PROC SQL;
   ...
   CREATE TABLE BASE_FALECOM_BIC_PF_ANTERIOR AS SELECT DISTINCT 
      ...
      /*FROM DB2BIC.AUX_INRO_CLI_ANT t1*/
	  FROM DB2BIC.AUX_INRO_CLI_CMPR t1
      WHERE t1.CD_TRAN_INRO_SIS IN ("MIV10014") AND MONTH(t1.DT_INCL_REG_INRO) = 12 AND YEAR(t1.DT_INCL_REG_INRO) = 2018;
...
PROC SQL;
	  ...
	  IFN(t2.PREFIXO IS NOT MISSING, t2.PREFIXO, t1.CD_PRF_DEPE_CLI) AS PREFIXO,
	  IFN(t2.CARTEIRA IS NOT MISSING, t2.CARTEIRA, 0) AS CARTEIRA,
	  t1.PTL,
	  t1.MCI,
	  t1.TS_ABERTURA,
	  t1.TS_FECHAMENTO,
...
PROC SQL;
   ...
   CREATE TABLE FALECOM_11 AS SELECT DISTINCT
      ...
      IFN(START_MEDIO >= 120, 0, IFN(START_MEDIO >= 105, 5, IFN(START_MEDIO >= 90, 10, IFN(START_MEDIO >= 75, 15, IFN(START_MEDIO >= 60, 20, 25 ))))) AS PONTOS_FALECOM
...
PROC SQL;
CREATE TABLE TABELA_FINAL_12_ANT AS SELECT t1.PREFDEP, t1.CARTEIRA, IFN(t1.CARTEIRA = 0, t1.TD_SINERGIA, t1.TipDepCnx) AS TipDepCnx , t1.REALIZADO AS REALIZADO_REPRO
FROM TABELA_FINAL_11 t1
GROUP BY 1,2;
QUIT;


/*ANALISE DE COMPARACAO PARA O REPROCESSAMENTO*/
/*ANALISE DE COMPARACAO PARA O REPROCESSAMENTO*/
/*ANALISE DE COMPARACAO PARA O REPROCESSAMENTO*/
/*ANALISE DE COMPARACAO PARA O REPROCESSAMENTO*/
...
PROC SQL;
CREATE TABLE ACORDO_CARTEIRA_RLZ AS 
SELECT DISTINCT cd_uor_ctra AS UOR, nr_seql_ctra as CTRA, vl_rlzd_in as REALIZADO_ATUAL
FROM DB2ATB.vl_aprd_in_ctra
where aa_vl_aprd_in = 2019 and mm_vl_aprd_in = MONTH(&D1.) and  cd_in_mod_avlc = 11953;
QUIT;
...
PROC SQL;
CREATE TABLE ACORDO_CARTEIRA_1_RLZ AS 
SELECT DISTINCT input(t2.PREFDEP, 4.) AS PREFIXO, CTRA, REALIZADO_ATUAL
FROM ACORDO_CARTEIRA_RLZ t1
INNER JOIN IGR.IGRREDE_&ANOMES. t2 ON t1.UOR = input(t2.UOR, 9.);
QUIT;


PROC SQL;
CREATE TABLE ACORDO_PREFIXO_RLZ AS 
SELECT DISTINCT cd_uor AS UOR, 0 as CTRA, vl_rlzd_in as REALIZADO_ATUAL
FROM DB2ATB.vl_aprd_in_uor
where aa_vl_aprd_in = 2019 and mm_vl_aprd_in = MONTH(&D1.) and cd_In_mod_avlc in(11992, 11954, 11984, 12000, 11952);
QUIT;


PROC SQL;
CREATE TABLE ACORDO_PREFIXO_1_RLZ AS 
SELECT DISTINCT input(t2.PREFDEP, 4.) AS PREFIXO, CTRA, REALIZADO_ATUAL
FROM ACORDO_PREFIXO_RLZ t1
INNER JOIN IGR.IGRREDE_&ANOMES. t2 ON t1.UOR = input(t2.UOR, 9.);
QUIT;
...
CREATE TABLE ACORDO_RLZ AS
 
   SELECT * FROM ACORDO_CARTEIRA_1_RLZ
   OUTER UNION CORR
   SELECT * FROM ACORDO_PREFIXO_1_RLZ;

Quit;


PROC SQL;
CREATE TABLE ACORDO_1_RLZ AS 
SELECT DISTINCT PREFIXO AS PREFDEP, t1.CTRA AS CARTEIRA, t1.REALIZADO_ATUAL
FROM ACORDO_RLZ t1
WHERE t1.REALIZADO_ATUAL <> 0
order by 1,2;
QUIT;


/*RELIZADO REPROCESSADO*/
/*RELIZADO REPROCESSADO*/


DATA PREPROCESSADO_1;
	MERGE TABELA_FINAL_12_ANT ACORDO_1_RLZ;
	BY PREFDEP CARTEIRA;
RUN;


PROC STDIZE DATA=PREPROCESSADO_1 OUT=PREPROCESSADO_1 REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;


PROC SQL;
CREATE TABLE PREPROCESSADO_2 AS 
SELECT DISTINCT t1.PREFDEP, t1.CARTEIRA, t1.TipDepCnx, IFN(t1.REALIZADO_ATUAL >= t1.REALIZADO_REPRO, t1.REALIZADO_ATUAL, t1.REALIZADO_REPRO) FORMAT 32.2 AS REALIZADO, 
(t1.REALIZADO_REPRO - t1.REALIZADO_ATUAL) FORMAT 32.2 AS DIFERENCA
FROM PREPROCESSADO_1 t1
order by 1,2;
QUIT;


PROC SQL;
CREATE TABLE TABELA_FINAL_12 AS 
SELECT DISTINCT t1.PREFDEP, t1.CARTEIRA, t1.TipDepCnx, t1.REALIZADO
FROM PREPROCESSADO_2 t1
order by 1,2;
QUIT;
...
PROC SQL;
    CREATE TABLE CONEXAO AS
        SELECT
            ...
            ||'F7176219'
            ||COMPRESS(PUT(&D1., ddmmyy10.))
            ||'N' AS L
        FROM TABELA_FINAL_12 t1       
;QUIT;


/*%GerarBBM(TabelaSAS=CONEXAO, Caminho=/dados/infor/transfer/enviar/, ExtencaoBBM=M0162);*/
