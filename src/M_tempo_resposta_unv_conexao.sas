...
PROC SQL;
   CREATE TABLE WORK.CARTEIRAS AS SELECT
      ...
      FROM WORK.nova_dependencias t2
           INNER JOIN COMUM.PAI_REL_PJ_&ANOMES. t1 ON (t2.prefixo = t1.CD_PRF_DEPE)
...
PROC SQL;
   CREATE TABLE WORK.CTRA_CLI_FC_ANTIGO AS 
   SELECT DISTINCT t2.CD_PRF_DEPE AS PREFIXO, 
      ...
      WHERE DATEPART(t1.TS_CRIC_MSG) >= MDY(MONTH('31dec2018'd),1,YEAR('31dec2018'd)) AND t1.NR_MTC_RPRT_CTRA = 0 
AND DATEPART(t1.TS_CRIC_MSG) <= '31dec2018'd;
QUIT;
...
PROC SQL;
   ...
   CREATE TABLE CTRA_CLI_FC AS 
      SELECT DISTINCT 

          t3.CD_PRF_DEPE AS PREFIXO, 
          t3.NR_SEQL_CTRA AS CARTEIRA,
		  t3.CD_CLI
      ...
      INNER JOIN COMUM.PAI_REL_PJ_&ANOMES. t3 ON (t2.CD_CLI_PJ = t3.CD_CLI)
...
DATA ;
	...
	MERGE CTRA_CLI_FC_ANTIGO CTRA_CLI_FC ;
	BY PREFIXO CARTEIRA;
...
PROC SQL;
   CREATE TABLE WORK.QUERY_FOR_TX_MSG_EXPS_ANTIGO AS 
   SELECT DISTINCT t1.CD_CLI_MSG_EXPS AS mci, 
          ...
          IFC(t1.NR_MTC_RPRT_CTRA = 0, "",'F'||PUT(t1.NR_MTC_RPRT_CTRA, Z7.)) AS responsavel, 
		  ...
		  t2.NR_SEQL_CTRA AS CARTEIRA
      FROM DB2MIV.TX_MSG_EXPS_PJ t1
	  INNER JOIN COMUM.PAI_REL_PJ_201812 t2 ON (t1.CD_CLI_MSG_EXPS = t2.CD_CLI)
      WHERE DATEPART(t1.TS_CRIC_MSG) >= MDY(MONTH('31DEC2018'd),1,YEAR('31DEC2018'd)) AND  DATEPART(t1.TS_CRIC_MSG) <= '31DEC2018'd
...
PROC SQL;
   CREATE TABLE QUERY_FOR_TX_MSG_EXPS AS 
...
PROC SQL;
   ...
   CREATE TABLE QUERY_FOR_TX_MSG_EXPS AS SELECT *
   FROM QUERY_FOR_TX_MSG_EXPS 
   OUTER UNION CORR SELECT * FROM QUERY_FOR_TX_MSG_EXPS_ANTIGO 
...
PROC SQL;
   ...
   SELECT t2.PREFIXO,
      ...
      FROM UNC_GAT.tel&AAAAMM. t1
	  INNER JOIN WORK.MCI_CTRA t2 ON (t1.MCI = t2.MCI)
	  WHERE t1.statusAtend = 40	AND t1.horaSolicitacaoAtend BETWEEN "8:0:0"t AND "18:0:0"t AND t1.dataFimAtend<=&D1.;
QUIT;
...
%SumarizadorCNX( TblSASValores=CONSOLIDADO_CNX,  TblSASColunas=COLUNAS_SUMARIZAR,  NivelCTRA=1,  PAA_PARA_AGENCIA=0,  TblSaida=CONSOLIDADO_CNX, AAAAMM=&ANOMES.); 
 

PROC SQL;
CREATE TABLE WORK.CONSOLIDADO_CNX AS 
SELECT POSICAO, PREFDEP, CTRA as Carteira, &ANOMES. AS ANOMES, TP_CNX_AVL as TipDepCnx
FROM  WORK.CONSOLIDADO_CNX;
QUIT;
...
INNER JOIN IGR.IGRREDE_&ANOMES. t2
...
PROC SQL;
CREATE TABLE auxiliar.CONSOLIDADO_REL AS 
SELECT *
FROM  WORK.CONSOLIDADO_REL;
QUIT;
