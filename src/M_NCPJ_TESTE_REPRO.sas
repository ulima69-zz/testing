      ...
      LEFT JOIN (SELECT B.CD_PSS_CTR_OPR AS CD_CLI
                    FROM DB2OPR.CTR_OPR A
                        INNER JOIN DB2OPR.PRTC_PSS_CTR_OPR B ON (A.NR_UNCO_CTR_OPR = B.NR_UNCO_CTR_OPR)
                            ...
                            WHERE A.CD_PRD = 6 and b.NR_SEQL_PRTC = 1 AND 
...
((A.DT_FRMZ >= '01.07.2018' AND A.DT_FRMZ <= '31.12.2018') OR (A.DT_FRMZ >= '01.01.2019' AND MONTH(A.DT_FRMZ) <= (&MES_POSICAO. -2)) OR (A.DT_FRMZ < '01.07.2018' AND B.DT_FIM_PRTC >= '01.07.2018'))
     ...
     (MONTH(t1.DEB307_DT_ABERTURA) = (&MES_POSICAO.) OR MONTH(t1.DEB307_DT_ABERTURA) = (&MES_POSICAO.-1)) AND YEAR(t1.DEB307_DT_ABERTURA) = 2019
...
AND	((A.DT_FRMZ >= '01.07.2018' AND A.DT_FRMZ <= '31.12.2018') OR (A.DT_FRMZ >= '01.01.2019' AND MONTH(A.DT_FRMZ) <= (&MES_POSICAO. -2)) OR (A.DT_FRMZ < '01.07.2018' AND B.DT_FIM_PRTC >= '01.07.2018'))
            ...
            WHERE t1.CD_TIP_PSS = 2
            AND (MONTH(t1.DT_ULT_RTVC_CT) = (&MES_POSICAO.) OR MONTH(t1.DT_ULT_RTVC_CT) = (&MES_POSICAO.-1)) AND YEAR(t1.DT_ULT_RTVC_CT) = 2019
            AND t2.DEB307_COD_BDC <> 0
            AND t4.CD_CLI IS NULL
...
PROC SQL;
   ...
   CREATE TABLE BASE_TFA_2_ANT AS SELECT DISTINCT    
      ...
      AND ((MONTH(t1.DT_INI) = (&MES_POSICAO.) OR MONTH(t1.DT_INI) = (&MES_POSICAO.-1)) AND YEAR(t1.DT_INI) = 2019) AND t1.DT_INI<= &D1.;
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_PJ_AGRUPANDO_1 AS SELECT DISTINCT  
	...
	MAX(t1.NR_SEQL_FATM) AS NR_SEQL_FATM
	  ...
	  WHERE (MONTH(t1.DT_REF_FATM) = 01 OR MONTH(t1.DT_REF_FATM) = 02) AND YEAR(t1.DT_REF_FATM) = 2019
	  GROUP BY 1
	  ORDER BY 1
      ;
    ...
    CREATE TABLE CANAIS AS
        ...
        FROM CONNECTION TO DB2(
            ...
            WHERE
                ...
                AND (MONTH(t1.TS_INRO_CLI) = (&MES_POSICAO.) OR MONTH(t1.TS_INRO_CLI) = (&MES_POSICAO.-1)) AND YEAR(t1.TS_INRO_CLI) = &ANO_ATUAL.   
...
PROC SQL;
	CREATE TABLE PGT_FILTRADA AS
		...
		INNER JOIN (SELECT CD_CLI, CD_CTR_PGTO, MIN(DT_CTR_PGTO) AS K
					...
					WHERE YEAR(DT_CTR_PGTO) = &ANO_ATUAL. AND (MONTH(DT_CTR_PGTO) = (&MES_POSICAO.-1) OR MONTH(DT_CTR_PGTO) = (&MES_POSICAO.)) AND DT_CTR_PGTO <= &D1. 
		ORDER BY 1, 2;
QUIT;
...
PROC SQL;
      ...
      WHERE t2.COD_PROD = 539 AND (MONTH(t2.data) = (&MES_POSICAO.-1) OR MONTH(t2.data) = (&MES_POSICAO.)) AND YEAR(t2.data) = &ANO_ATUAL. AND t2.data <= &D1.;
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_FINAL_550 AS 
      SELECT DISTINCT 
		 ...
		 t1.FOPAG,
         t1.OP_CREDITO,
		 IFC(t1.VALENDO = 1 AND t1.QUALIFICADO = "Sim", "Sim", "Não") as QUALIFICADO,
         IFN(t1.VALENDO = 1 AND t1.QUALIFICADO = "Sim", t1.PONTOS, 0) as PONTOS,
...
PROC SQL;
   ...
   SELECT DISTINCT 
       ...
       FROM COX.cli_ind_000000180_dt012019 t1
 	   WHERE t1.COMP = 3 AND t1.MMAAAA = 012019
	   ORDER BY 1;
QUIT;
...
PROC SQL;
   ...
   CREATE TABLE CLIENTES_FINAL_328 AS 
      SELECT DISTINCT 
		 ...
		 t2.UOR,
         t1.VALENDO,
		 t1.PONTOS AS PTO_MES_ATU,
		 0 AS PTO_MES_ANT		 
         	  		            
      FROM CLIENTES_FINAL_550 t1
	  INNER JOIN PREFIXOS_IND_000000180 t2 ON t1.PREFDEP = t2.PREFDEP AND t1.CARTEIRA = t2.CTRA
...
PROC STDIZE DATA=CLIENTES_FINAL_328 OUT=CLIENTES_FINAL_328 REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
...
PROC SQL;
   ...
   SELECT  
       ...
       FROM COX.indicador_000000180 T1
 	   WHERE t1.COMP = 0 AND RM02 IS NOT MISSING
	   ORDER BY 1,2;
QUIT;


PROC STDIZE DATA=CONEX_FEV_ANT OUT=CONEX_FEV_ANT REPONLY MISSING=0;
	VAR _NUMERIC_;
QUIT;
...
PROC SQL;
   ...
   SELECT  
		  ...
		  t1.UOR,
		  t1.RM02 AS FEV_ATU,
          t1.CONTAGEM AS FEV_REPRO,
          (t1.CONTAGEM - t1.RM02) as DIFERENCA
		  /*IFN(t1.RM02 > t1.CONTAGEM, t1.RM02, t1.CONTAGEM) AS contagem*/		 
