...
PROC SQL;
CREATE TABLE JANEIRO_REPRO AS 
SELECT INPUT(prefdep, d4.) as PREFIXO, carteira AS CTRA, valor as REALIZADO_JAN_REPRO
FROM valor_fim;
QUIT;
...
SELECT PREFIXO, CTRA, REALIZADO_JAN_ATUAL FORMAT 32.2, REALIZADO_JAN_REPRO FORMAT 32.2, 

IFN((REALIZADO_JAN_REPRO - REALIZADO_JAN_ATUAL) > 0.5 OR (REALIZADO_JAN_REPRO - REALIZADO_JAN_ATUAL) 
< -0.5, (REALIZADO_JAN_REPRO - REALIZADO_JAN_ATUAL), 0) AS DIFERENCA,

IFN(REALIZADO_JAN_REPRO > REALIZADO_JAN_ATUAL, REALIZADO_JAN_REPRO, REALIZADO_JAN_ATUAL) FORMAT 32.2 AS DEFINITIVO
